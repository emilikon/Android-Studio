...
class DataSource {
    ...
    public suspend fun fetchVotes() : List<AlpacaVotes>? {
        val path = "https://www.uio.no/studier/emner/matnat/ifi/IN2000/v22/obligatoriske-oppgaver/district1.json"
        val gson = Gson()

        try {
            val responseVotes = gson.fromJson(Fuel.get(path).awaitString(), BaseVotes :: class.java)
            return responseVotes.votes
        }catch (e: Exception){
            println(e.message)
            return null
        }
